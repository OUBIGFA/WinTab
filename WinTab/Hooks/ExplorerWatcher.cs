...
public class ExplorerWatcher : IHook
{
    ...
    public async Task SelectTabByHandle(nint windowHandle, nint tabHandle)
    {
        ...
        if (targetIndex != -1)
        {
            SelectTabByIndex(windowHandle, targetIndex);
            
            // Wait for it to become the top-most child (active)
    ...
    public async void OpenNewTab(nint windowHandle, string location)
    {
        await OpenTabNavigateWithSelectionAndFocus(new WindowRecord(location, windowHandle), windowHandle);
    }
    ...
    private async void OnShellWindowRegistered(int __)
    {
        ...
        try
        {
            ...
            if (shouldReopenAsTab && IsTemporaryLocation(location))
                location = await ResolveWindowLocationAsync(window, location, 2000);
            ...
            // If it's a new tab (not a redirected window) and the setting is enabled
            if (!shouldReopenAsTab && 
                SettingsManager.OpenCurrentTabOnNewTab && 
                IsTemporaryLocation(location))
            {
                var activeTabHandle = GetActiveTabHandle(_mainWindowHandle);
                if (activeTabHandle != 0 && activeTabHandle != hWnd)
                {
                    var activeWindow = GetWindowByTabHandle(activeTabHandle);
                    if (activeWindow != null)
                    {
                        var currentPath = GetLocation(activeWindow);
                        if (!string.IsNullOrWhiteSpace(currentPath))
                        {
                            _ = Navigate(window, currentPath);
                        }
                    }
                }
            }
    ...
    private async Task RestorePreviousWindows()
    {
        var result = await RunInStaThread(() => CustomMessageBox.Show(
            (string)Application.Current.FindResource("Message_RestoreWindows"),
            Constants.AppName,
    ...
    private async Task OpenTabNavigateWithSelection(WindowRecord windowToOpen, nint windowHandle = 0, bool isDuplicate = false, bool forceTabReuse = false)
    {
        ...
        try
        {
            ...
            // Bring the window to front first
            WinApi.RestoreWindowToForeground(mainWindowHWnd);

            // Auto-focus to the new tab immediately if it was opened from sidebar (middle-click)
            await SelectTabByHandle(mainWindowHWnd, newTabHandle);
    ...
    private async Task OpenTabNavigateWithSelectionAndFocus(WindowRecord windowToOpen, nint windowHandle)
    {
        await OpenTabNavigateWithSelection(windowToOpen, windowHandle, isDuplicate: false, forceTabReuse: false);
    }
    ...
    private bool IsTemporaryLocation(string location)
    {
        if (string.IsNullOrWhiteSpace(location)) return true;

        if (string.Equals(location, "about:blank", StringComparison.OrdinalIgnoreCase))
            return true;

        return string.Equals(location, _defaultLocation, StringComparison.OrdinalIgnoreCase);
    }
    private async Task<string> ResolveWindowLocationAsync(InternetExplorer window, string location, int timeoutMs)
    {
        if (!IsTemporaryLocation(location)) return location;
        ...
        navigateHandler = (object _, ref object _) =>
        {
            var updated = TryGetLocation(window);
            if (IsTemporaryLocation(updated)) return;
        ...
        var pollTask = Task.Run(async () =>
        {
            ...
            {
                var updated = TryGetLocation(window);
                if (!IsTemporaryLocation(updated))
                {
                    tcs.TrySetResult(updated);
                    return;
                }
        ...
        var latest = TryGetLocation(window);
        return !string.IsNullOrWhiteSpace(latest) ? latest : location;
    }

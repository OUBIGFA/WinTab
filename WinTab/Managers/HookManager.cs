...
public sealed class HookManager
{
    ...
    private readonly WindowHostManager _windowHostManager;
    ...
    public HookManager(ProfileManager profileManager)
    {
        ...
        _windowHostManager = new WindowHostManager(_windowHook);
        _tabEngine = new TabEngine(_windowHostManager);
        _mouseHook = new Mouse(profileManager.GetProfiles());
        _keyboardHook = new Keyboard(profileManager.GetProfiles());

        _windowHostManager.WindowCreated += entry => _tabEngine.NotifyCreated(entry);
        _windowHostManager.WindowDestroyed += hWnd => _tabEngine.RemoveWindow(hWnd);
        _windowHostManager.WindowActivated += hWnd => _tabEngine.NotifyActivated(hWnd);
    ...
    public void StartGeneralWindowHost() => _windowHostManager.StartGeneral();
    public void StopGeneralWindowHost() => _windowHostManager.StopGeneral();
    internal IReadOnlyCollection<WindowEntry> GetGeneralWindows() => _windowHostManager.GetGeneralWindows();
    public TabEngine GetTabEngine() => _tabEngine;
    ...
    private async void OnHotKeyProfileTriggered(HotKeyEventArgs e)
    {
        switch (e.Profile.Action)
        {
            ...
            case HotKeyAction.TabSearch:
                _syncContext.Post(_ => new TabSearchPopup(_tabEngine).Show(), null);
                break;

...
public partial class HotKeyProfileControl : UserControl
{
    ...
    private void InitializeControls()
    {
        ...
        {
            var key = $"HotkeyScope_{scope}_Display";
            var label = Application.Current.Resources.Contains(key)
            ...
            CbScope.Items.Add(item);
            if (scope == _profile.Scope)
                CbScope.SelectedItem = item;
        }
    ...
    {
        if (CbScope.SelectedItem is ComboBoxItem { Tag: HotkeyScope scope })
    ...
    private void UpdateAction()
    {
        ...
        UpdateActionToolTip(selectedAction);
    }
    ...
    private void UpdateActionToolTip(HotKeyAction action)
    {
        var key = $"HotKeyActionDesc_{action}";
        if (Application.Current.Resources.Contains(key))
        {
            CbAction.ToolTip = Application.Current.FindResource(key);
            return;
        }

        CbAction.ClearValue(ToolTipProperty);
    }

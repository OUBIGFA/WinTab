...
internal static class ExplorerStressTest
{
    ...
    public static async Task<int> RunActivationAsync(string[] args)
    {
        ...
        try
        {
            ...
            var tabs = ExplorerWindowDiscovery.GetAllExplorerTabs(mainHwnd).ToArray();
    ...
    {
        var knownTabs = ExplorerWindowDiscovery.GetAllExplorerTabs(mainHwnd).ToArray();
        for (var i = 0; i < additionalTabCount; i++)
        {
            ...
            var newTab = await ExplorerWindowDiscovery.ListenForNewExplorerTabAsync(mainHwnd, knownTabs, 3_000);
            if (newTab == 0)
                return false;

            knownTabs = ExplorerWindowDiscovery.GetAllExplorerTabs(mainHwnd).ToArray();
            await Task.Delay(75);
        }
    ...
    private static void ConcealExplorerWindowAsOrphan(nint hWnd)
    {
        ...
        ExplorerWindowVisibility.UpdateLayeredStyle(hWnd, remove: false);
    ...
    private static void TryRestoreExplorerWindow(nint hWnd)
    {
        ...
        try
        {
            ...
            ExplorerWindowVisibility.UpdateLayeredStyle(hWnd, remove: true);
    ...
    private static void DumpWindowVisibility(nint hWnd)
    {
        ...
        Console.Error.WriteLine($"HWND={hWnd} Explorer={ExplorerWindowDiscovery.IsFileExplorerWindow(hWnd)} Visible={WinApi.IsWindowVisible(hWnd)} Layered={layered} Alpha={alphaText} Rect={rectText}");
    }
    ...
    {
        var tabs = ExplorerWindowDiscovery.GetAllExplorerTabs(windowHandle).ToArray();
        if (tabs.Length < 2)
            return false;
    ...
    private sealed class VisibleExplorerWindowMonitor
    {
        ...
        private async Task PollAsync(CancellationToken cancellationToken)
        {
            while (!cancellationToken.IsCancellationRequested)
            {
                foreach (var window in ExplorerWindowDiscovery.GetAllExplorerWindows())
                {

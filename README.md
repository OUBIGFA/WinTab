...
WinTab 运行时默认持续记录标签组，无需开启自动恢复：正常关闭时保存最后关闭的窗口；关机或资源管理器崩溃时，保留最后使用、尚未关闭窗口的最近完整记录。只恢复一个窗口，不会恢复全部历史窗口。「恢复单个标签」默认关闭，此时仅保存和恢复含两个及以上标签的窗口，关闭单标签窗口不会覆盖已保存的组。

托盘的独立「标签恢复」功能区和设置页均提供：

- **恢复上次标签组**：直接恢复到一个新的独立窗口，保留可恢复标签的顺序及原活动标签；默认快捷键 `Ctrl+Shift+E`。
- **恢复上一个标签页**：恢复最近关闭的标签，优先使用当前资源管理器窗口；默认开启，默认快捷键 `Ctrl+Shift+T`。最近 25 个关闭记录仅保留在本次 WinTab 运行期间，关闭此功能会清空记录。启用标签复用时，已打开的位置会直接切换过去。

两个快捷键都可单独关闭或在设置页修改，仅在资源管理器处于前台时生效，不占用浏览器快捷键。路径记录保存在 `%APPDATA%\WinTab\session.json` 和 `session-live.json`（及其备份），请按个人文件夹信息保护这些文件。

### 自动恢复（可选）

「自动恢复上次标签组」默认关闭。开启后，仅当没有其他资源管理器窗口时，下次新打开的窗口才可能触发恢复，与「合并新窗口」独立：
